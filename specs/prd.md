...
> **口径注记（Phase F 实测修正，2026-08-15）**：额外开销的分母起点取**故障可观测时刻 T0'** 而非注入时刻 T0——
> 注入手法固有的「故障爬坡」（kubelet STOP→节点 condition 翻转 ~40-55s、CrashLoopBackOff 退避、Pending 调度等待）
> 是 K8s 机制，不属于 §11.1 分解式中的告警链路四段（scrape/for/group_wait/送达）。
> 即：`额外开销 = T_detect − T0' − for`。Phase F 三类 ×5 实测（重建口径）链路自身开销均 ≤60s 过门；
> 原始逐轮数据与分段预算见 `docs/phase-manuals/phase-F-MTTD-数据.md`。T0' 由注入侧事件时间或分段常量预算推得。
