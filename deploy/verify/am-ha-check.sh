...
pdb_name=$(kubectl -n monitoring get pdb -o name 2>/dev/null | grep alertmanager | head -1)
[ -n "$pdb_name" ] || { echo "未找到 AM 的 PDB"; exit 1; }
pdb=$(kubectl -n monitoring get "$pdb_name" -o jsonpath='{.spec.minAvailable}' 2>/dev/null)
