...
- **v1.2**（2026-08-14，Task 0 预演实测回填）：**Task 0 Step 3 两处命令修正**（不改 v1.1 架构，仅修预演踩到的命令 bug）：
  ...
  6. **Task 4 busybox nc 命令修 3 处**（plan Step 1 manifest `nc -l -p 8080 -q 1`）：busybox:1.38.0 nc 实测：① `-q 1` 不支持（`nc -h` 无 `-q`，报 `punt!`）→ 删；② 任何 `-w N` 让 idle listener 每 N 秒超时退出→loop 重绑产生 ~0.5-1s "connection refused" 空窗（单次 wget ~20% 被拒）→ 删 `-w`，idle listener 持续 LISTEN；③ plan 的 printf 只发 header+Content-Length **body 从未送出**（client `connection closed prematurely`）→ 改 `( printf headers; cat /tmp/resp ) | nc -l -p 8080` 把 body 拼进同一管道。未用 httpd fallback（保留 nc 架构 + 可控 Content-Type）。端点 5/5 稳定。Task 4 commit `7071b25`。
  7. **Task 4 流程顺序**（新源文件）：plan Step 4 apply→verify 在 Step 5 commit→re-sync 前，但**新源文件（sms-provider/manifest.yaml）须先 commit+`git push /srv/git/k8s-monitor.git HEAD:main` 再 apply Application**，否则 ArgoCD `ComparisonError: app path does not exist`。正确序：写文件→commit→re-sync 裸仓→apply Application→等 reconcile→verify。（Task 2/3 源文件已存在 main 故无此问题；Task 4/6 新文件须遵循。）
  8. **Task 5 silence.sh 修 4 处**（plan Step 1/2 命令）：
     - ① **`kubectl get --raw ... -X POST -d` 不支持**：`get --raw` 仅 GET，`-X` 报 `unknown shorthand flag: 'X' in -X`（实测复现）。改 **port-forward + curl**（随机端口，自管生命周期）。**⚠️ 此坑直接影响 Task 9**：plan measure-mttd-batch.sh Step 1 的 auto-silence 用同样的 `kubectl get --raw ... -X POST` → 必改用 `silence.sh create` 或 port-forward+curl。
     - ② **silences POST body 是【单对象 {…}】非数组**：实测 `[{...}]` → HTTP 400 `cannot unmarshal array into struct`。AM v2：`/api/v2/alerts` 收数组 `[...]`，`/api/v2/silences` 收单对象 `{...}`（API 设计差异，勿混）。**⚠️ Task 9 auto-silence body 同改单对象。**
  ...
  10. **Task 9 长跑+取证（2026-08-15 收尾）**：批量原始数字 9/15 轮被「上一轮 resolved 卡片」污染（AM send_resolved:true + group_interval 5m flush 落在下一轮 T0 后，被 T_detect=「ts≥T0 首条 send」误判）。取证+离线重建（T_detect=首条 ts≥T0+for）：not-ready 426/430、crashloop 709/757、pod-pending 677/682，送达 15/15=100%。measure-mttd.sh 加 MIN_WAIT 约束（`8aa6796`）。**北极星口径修正（用户决策）**：额外开销 = T_detect − T0' − for（T0'=故障可观测时刻；爬坡+采集周期是 K8s 机制非链路），链路自身 30-56s 全过门；PRD §11.1 加注记；数据 `docs/phase-manuals/phase-F-MTTD-数据.md`。
  11. **Task 10**：三场景全 23/0（verify-all 实际输出非 recover 自报）。场景① 真实走通 L1（检出 4 失败→restart kindnet/kube-proxy，覆盖 NodePort wedge quirk）；场景③ wedge 未复现（kind#2045 非必现），幂等验证+wedge 自愈以场景① L1 为等效证据。开机后立即 kubectl wait 可能报 apiserver RBAC 未就绪，等 20s 重试。
  12. **Task 11 受控偏离（argocd Ingress 不重建）**：host argocd.local 已被 Helm release 管的现存 `argocd-server` Ingress 占用（backend port 80 明文）。plan verbatim 再建同 host 同 path → ingress-nginx 双 backend 轮询（80 明文 vs 443 TLS）间歇 502；且 backend-protocol:HTTPS 无必要。只新建 alertmanager Ingress；ArgoCD 可达性走现存 Ingress 断言。另：**ingress-nginx 无 NodePort service**（controller hostNetwork+hostPort 80/443 @ control-plane），assert BASE=http://localhost（同 verify-all echo 检查路径）。
