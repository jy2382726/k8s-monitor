# Phase F · GitOps + 收尾 操作手册（定稿 · 用户复现版）

> **版本**：定稿（2026-08-15，闭环③）。基于 agent 预演（12/12 Task + 9 AC 全过）定稿；
> 预演期原始记录见草稿 `phase-F-操作手册-草稿.md`（保留不覆盖，回看 agent 原始记录用）。
...
> **MTTD 数据**：`docs/phase-manuals/phase-F-MTTD-数据.md`（§13 表的逐轮明细/取证证据/分段预算）
...
> **git 前置（定稿时已做）**：Phase F 全部 commit 已合并 origin/main（至 `c323282`）——
> 用户在主仓 `/root/projects/k8s-monitor` 直接 `git pull` 即拿到本文引用的全部文件。
> **复现边界（降级）**：MTTD 每类抽验 1 次（§13.5）｜Watchdog 1h 心跳 Phase D 已验｜
> oom / control-plane 受控偏离⑤①（验规则在位即可）｜AC-US2/US4/US5 前序闭环不重验。
...
生产割接从这个态出发。git 层收尾（合并 main + push origin，定稿时已完成至 `c323282`）
只涉仓库不动集群；用户复现通过后无其他集群动作。
